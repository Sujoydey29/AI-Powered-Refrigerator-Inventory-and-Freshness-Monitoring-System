...
We leverage Ultralytics YOLO11 for both detection and classification tasks, using a combination of public datasets.
...
- **How**:  
  ```bash
  yolo classify train     data=fruits360.yaml     model=yolo11n-cls.pt     epochs=50     imgsz=224     batch=64     name=fruits360-cls
  ```
...
* **How**:
  ...
  1. **Export to yolov11 format** via FiftyOne  
     ```python
     import fiftyone.zoo as foz
     from fiftyone.types import yolov11Dataset
     ...
     ds.export(
       ...
       dataset_type=yolov11Dataset,
       label_field="ground_truth",
       classes=FRUIT_CLASSES,
       overwrite=True,
     )
     ```
  2. **Train YOLO11 detection**  
     ```bash
     yolo detect train        data=open_images_fruits.yaml        model=yolo11n.pt        epochs=30        imgsz=640        batch=16        device=0        half=True        name=stage2-openimages-fruits
     ```
